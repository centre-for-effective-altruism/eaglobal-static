...
// override class defaults to support HTTPS base URL for youtube embeds
urlEmbed.defaultProviderClasses.Youtube.prototype.providerURL =
...
console.log(
  chalk.inverse("Searching for Contentful entries with missing embed data...")
);
contentful
      ...
      .then(function (entries) {
        ...
        entries.items.forEach(function (entry) {
          ...
          if (
            ...
            (!entry.fields.thumbnail || !entry.fields.oembed)
          ) {
            entriesMissingData.push(entry);
          }

          if (
            parseInt(entry.sys.publishedVersion, 10) ===
            parseInt(entry.sys.version, 10) - 1
          ) {
        ...
        console.log(
          chalk.cyan("+"),
          "Found",
          chalk.cyan(entriesMissingData.length),
          "entries with missing embed data"
        );
        ...
        return new Promise((resolve, reject) => {
            ...
            entriesMissingData.forEach((entry, i) => {
              ...
              if (!entry.fields.thumbnail) {
                ...
                if (!imageUrl) {
                  ...
                  throw new TypeError(
                    imageUrl + " is an invalid value for imageUrl"
                  );
                }
                assets.push(
                  contentful.formatItems({
          ...
          .then(function () {
            return contentful
              .itemQueue("update", entriesMissingData)
              .then(function (entries) {
                console.log(tick, "Updated entries");
                // filter any entries that aren't currently published and at the latest version
                var entriesToPublish = entries.filter(
                  (entry) => publishedEntryIDs.indexOf(entry.sys.id) !== -1
                );
                ...
                return contentful
                  .itemQueue("publish", entriesToPublish)
                  .then(function (entries) {
                    console.log(
                      tick,
                      "Published",
                      chalk.cyan(entries.length),
                      "updated entries"
                    );
                  });
              });
